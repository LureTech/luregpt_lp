...
/**
 * Bloco 12 — conversão.
 * A referência fecha com um CTA de WhatsApp; o briefing pede o formulário
 * inteiro na tela, não um botão que rola para outro lugar. Fica o formulário.
 */
export default function Fechamento() {
  ...
  return (
    <section className="secao fechamento secao--clara" id="fechamento">
      <div className="grade-chao" />

      <div className="wrap fechamento__grade">
        <div>
          <Titulo partes={FECHAMENTO.titulo} nivel="h2" />
          <p className="sub" style={{ marginInline: 0 }}>
            {FECHAMENTO.sub}
          </p>

          <span className="parceiro-meta parceiro-meta--claro">
            <img src="/mock-meta.webp" alt="" width="22" height="22" />
            Parceiro oficial da Meta
          </span>

          <ul className="lista-seca">
            {FECHAMENTO.bullets.map((b) => (
              <li key={b}>{b}</li>
            ))}
          </ul>
        </div>
        ...
        <div className="form">
          {estado === "pronto" ? (
            <div className="form__ok">
              <h3>{FECHAMENTO.form.sucesso.titulo}</h3>
              <p>{FECHAMENTO.form.sucesso.texto}</p>
              {/* PENDENTE (P6): embutir a agenda aqui. */}
            </div>
          ) : (
            <form onSubmit={aoEnviar} noValidate>
              <h3 style={{ fontSize: "1.4rem" }}>{FECHAMENTO.form.titulo}</h3>
              <p
                style={{
              ...
              <div className="form__dupla">
                <div className="form__campo">
                  <label htmlFor="pessoas">Pessoas na empresa</label>
                  <select
                    id="pessoas"
                    required
                    value={dados.pessoas}
              ...
              <div className="form__campo">
                <label htmlFor="ferramenta">Qual ferramenta você usa hoje</label>
                <input
                  id="ferramenta"
                  required
